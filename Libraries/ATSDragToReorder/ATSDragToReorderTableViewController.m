...
- (void)establishGestures {
	...
	if (longPressGestureRecognizer == nil || [self.tableView.gestureRecognizers containsObject:longPressGestureRecognizer] == NO) {
		...
		//longPressGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(longPressRecognized)];
...
- (NSIndexPath *)anyIndexPathFromLongPressGesture {
	...
	for (NSUInteger pointIndex = 0; pointIndex < [longPressGestureRecognizer numberOfTouches]; ++pointIndex) {
		...
		CGPoint relativePoint = CGPointMake(touchPoint.x - cell.frame.origin.x, touchPoint.y - cell.frame.origin.y);
		
		if ((relativePoint.x < self.touchArea.origin.x) ||
			(relativePoint.y < self.touchArea.origin.y) ||
			(relativePoint.x >= self.touchArea.origin.x + self.touchArea.size.width) ||
			(relativePoint.y >= self.touchArea.origin.y + self.touchArea.size.height))
			indexPath = nil;
